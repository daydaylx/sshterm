...
class InputController @Inject constructor(
    ...
    fun shouldHandleKeyEvent(event: KeyEvent): Boolean {
        return event.action == KeyEvent.ACTION_DOWN && !keyMapper.isSystemKey(event.keyCode)
    }
    ...
    suspend fun handleSoftwareInput(text: String) {
        withContext(dispatchers.io) {
            ...
            writeSoftwareText(text, activeModifiers)
        }
    }
    ...
    private suspend fun writeSoftwareText(
        text: String,
        activeModifiers: Set<ModifierKey>
    ) {
        val plainText = StringBuilder()

        suspend fun flushPlainText() {
            if (plainText.isNotEmpty()) {
                if (activeModifiers.isEmpty()) {
                    inputWriter.writeString(plainText.toString())
                } else {
                    plainText.forEach { char ->
                        write(keyMapper.mapCharWithModifiers(char, activeModifiers))
                    }
                }
                plainText.clear()
            }
        ...
        text.forEach { char ->
            ...
            if (specialKey == null) {
                ...
                flushPlainText()
                write(keyMapper.mapSpecialKey(specialKey))
            }
        }
